...
public class MoviesAdapter extends RecyclerView.Adapter<MoviesAdapter.MoviesViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MoviesViewHolder holder, int position) {
        String movieName = moviesResponseResults.get(position).getOriginal_title();
        ...
        if (moviesResponseResults.get(position).getRelease_date().length()>=1) {
            String date = moviesResponseResults.get(position).getRelease_date().substring(0, 4);
